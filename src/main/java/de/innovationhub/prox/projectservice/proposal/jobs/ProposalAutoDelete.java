...
@ConditionalOnProperty(
    prefix = "project-service.proposals.jobs.auto-delete",
    value = "enable",
    havingValue = "true",
    matchIfMissing = true)
@Slf4j
public class ProposalAutoDelete {
  ...
  void autoDelete() {
    // Just use the current timestamp to ensure forward-compatibility as we might introduce
    // pre-dated
    // drafts
    var qualifyingTimestamp = Instant.now();
    var proposalsToDelete =
        this.proposalRepository.findWithStatusModifiedBefore(
            ProposalStatus.READY_FOR_DELETION, qualifyingTimestamp);
