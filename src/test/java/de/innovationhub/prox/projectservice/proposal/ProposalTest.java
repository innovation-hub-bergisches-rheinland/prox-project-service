...
class ProposalTest {
  ...
  void shouldUpdateTimestampWhenStatusChanged() throws InterruptedException {
    var proposal =
        Proposal.builder().name("Test").status(ProposalStatus.PROPOSED).requirement("Test").build();
    ...
    // Then
    assertThat(proposal.getStatusChangedAt()).isAfter(firstStatusChangedTimestamp);
  }
}
